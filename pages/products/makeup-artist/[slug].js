...
function ViewPhotography({posts}){
  const data = posts
    return(
 <>
    <Main data={data} service={services.makeupArtist}/>
 </>
    )
}
...
export async function getStaticProps({params}) {
  console.log(params); 
    const res =  await API.graphql({
      query: queries.getMakeupArtist,
      variables: { id: params.slug },
      authMode: 'API_KEY'
    });
    const posts = await res?.data?.getMakeupArtist
  console.log(posts)
    return {
      props: {
        posts,
...
export async function getStaticPaths() {
  ...
  const res = await SSR.API.graphql({
    ...
    const photographer = await res?.data?.listMakeupArtists?.items
